    ...
    public void Extract(IXLWorkbook workbook, ExtractionContext context)
    {
        foreach (var row in workbook.GetDataRows("Classes", isRequired: true))
        {
            var classDef = new ClassDefinition
            {
                ...
                PrimaryAbility = row.Cell(Col.PrimaryAbility).GetEnumList<AttributeImprovementChoice>(),
                SavingThrowProficiencies = row.Cell(Col.SavingThrowProficiencies).GetEnumList<AttributeImprovementChoice>(),
